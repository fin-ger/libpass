use std::{fmt, fs::{File, OpenOptions}, io::Write, path::{Path, PathBuf}};
...
impl DecryptedPassword {
    pub(crate) fn from_path(path: &Path) -> Result<Self, StoreError> {
        ...
        let mut ctx = Context::from_protocol(Protocol::OpenPgp).with_store_error(path.display().to_string())?;
    ...
    fn save(&self) -> Result<(), StoreError> {
        ...
        f.write_all(format!("{}", self).as_bytes()).with_store_error(self.path.display().to_string())?;
    ...
    pub fn replace_line<L: Into<String>>(&mut self, position: Position, line: L) -> Result<Option<String>, StoreError> {
    ...
    pub fn insert_line<L: Into<String>>(&mut self, position: Position, line: L) -> Result<(), StoreError> {
        let old_lines = self.lines.clone();
        self.lines.insert(position, line.into());
        match self.save() {
            Ok(()) => Ok(()),
